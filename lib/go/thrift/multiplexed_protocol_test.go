...
const MULTIPLEXED_SERVICE_NAME = "test_service"

func NewTMultiplexedProtocol(trans TTransport) TProtocol {
	return &TMultiplexedProtocol{
		TProtocolDecorator{
			NewTSimpleJSONProtocol(trans),
		},
		MULTIPLEXED_SERVICE_NAME,
	}
}

func TestMultiplexedProtocolEntireMessage(t *testing.T) {
	trans := NewTMemoryBuffer()
	p := NewTMultiplexedProtocol(trans)
	const MSG_FN = "function_name"
	const MSG_TYPE = CALL
	const MSG_SEQ = 1
	const BOOL_VALUE = true
	thetype := "boolean"

	if e := p.WriteMessageBegin(MSG_FN, MSG_TYPE, MSG_SEQ); e != nil {
		t.Fatalf("Unable to write message begin due to error: %s", e.Error())
	}
	if e := p.WriteBool(BOOL_VALUE); e != nil {
		t.Fatalf("Unable to write %s value %v due to error: %s", thetype, BOOL_VALUE, e.Error())
	}
	if e := p.WriteMessageEnd(); e != nil {
		t.Fatalf("Unable to write message end due to error: %s", e.Error())
	}
	if e := p.Flush(); e != nil {
		t.Fatalf("Unable to write message begin due to error flushing: %s", e.Error())
	}

	s := trans.String()
	var expected = fmt.Sprintf("[\"%s:%s\",%d,%d,%v]", MULTIPLEXED_SERVICE_NAME, MSG_FN, MSG_TYPE, MSG_SEQ, BOOL_VALUE)
	if s != fmt.Sprintf(expected) {
		t.Fatalf("Invalid message %q; expected %q", s, expected)
	}
}

func TestWriteMultiplexedProtocolBool(t *testing.T) {
	thetype := "boolean"
	trans := NewTMemoryBuffer()
	p := NewTMultiplexedProtocol(trans)
...
func TestReadMultiplexedProtocolBool(t *testing.T) {
	thetype := "boolean"
	for _, value := range BOOL_VALUES {
		trans := NewTMemoryBuffer()
		p := NewTMultiplexedProtocol(trans)
...
func TestWriteMultiplexedProtocolByte(t *testing.T) {
	thetype := "byte"
	trans := NewTMemoryBuffer()
	p := NewTMultiplexedProtocol(trans)
...
func TestReadMultiplexedProtocolByte(t *testing.T) {
	thetype := "byte"
	for _, value := range BYTE_VALUES {
		trans := NewTMemoryBuffer()
		p := NewTMultiplexedProtocol(trans)
...
func TestWriteMultiplexedProtocolI16(t *testing.T) {
	thetype := "int16"
	trans := NewTMemoryBuffer()
	p := NewTMultiplexedProtocol(trans)
...
func TestReadMultiplexedProtocolI16(t *testing.T) {
	thetype := "int16"
	for _, value := range INT16_VALUES {
		trans := NewTMemoryBuffer()
		p := NewTMultiplexedProtocol(trans)
...
func TestWriteMultiplexedProtocolI32(t *testing.T) {
	thetype := "int32"
	trans := NewTMemoryBuffer()
	p := NewTMultiplexedProtocol(trans)
...
func TestReadMultiplexedProtocolI32(t *testing.T) {
	thetype := "int32"
	for _, value := range INT32_VALUES {
		trans := NewTMemoryBuffer()
		p := NewTMultiplexedProtocol(trans)
...
func TestWriteMultiplexedProtocolI64(t *testing.T) {
	thetype := "int64"
	trans := NewTMemoryBuffer()
	p := NewTMultiplexedProtocol(trans)
...
func TestReadMultiplexedProtocolI64(t *testing.T) {
	thetype := "int64"
	for _, value := range INT64_VALUES {
		trans := NewTMemoryBuffer()
		p := NewTMultiplexedProtocol(trans)
...
func TestWriteMultiplexedProtocolDouble(t *testing.T) {
	thetype := "double"
	trans := NewTMemoryBuffer()
	p := NewTMultiplexedProtocol(trans)
...
func TestReadMultiplexedProtocolDouble(t *testing.T) {
	thetype := "double"
	for _, value := range DOUBLE_VALUES {
		trans := NewTMemoryBuffer()
		p := NewTMultiplexedProtocol(trans)
...
func TestWriteMultiplexedProtocolString(t *testing.T) {
	thetype := "string"
	trans := NewTMemoryBuffer()
	p := NewTMultiplexedProtocol(trans)
...
func TestReadMultiplexedProtocolString(t *testing.T) {
	thetype := "string"
	for _, value := range STRING_VALUES {
		trans := NewTMemoryBuffer()
		p := NewTMultiplexedProtocol(trans)
...
func TestWriteMultiplexedProtocolBinary(t *testing.T) {
	thetype := "binary"
...
func TestReadMultiplexedProtocolBinary(t *testing.T) {
	thetype := "binary"
	value := protocol_bdata
	b64value := make([]byte, base64.StdEncoding.EncodedLen(len(protocol_bdata)))
	base64.StdEncoding.Encode(b64value, value)
...
func TestWriteMultiplexedProtocolList(t *testing.T) {
	thetype := "list"
	trans := NewTMemoryBuffer()
	p := NewTMultiplexedProtocol(trans)
...
func TestWriteMultiplexedProtocolSet(t *testing.T) {
	thetype := "set"
	trans := NewTMemoryBuffer()
	p := NewTMultiplexedProtocol(trans)
...
func TestWriteMultiplexedProtocolMap(t *testing.T) {
	thetype := "map"
	trans := NewTMemoryBuffer()
	p := NewTMultiplexedProtocol(trans)
